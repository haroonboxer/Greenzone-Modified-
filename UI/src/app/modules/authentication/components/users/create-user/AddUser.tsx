...
const AddUser = () => {
  ...
  const [selectedSignatureFiles, setSelectedSignatureFiles] = useState<File[]>([])
  const [signaturePreviews, setSignaturePreviews] = useState<string[]>([])
  const [SignatureFileTypes, setSignatureFileTypes] = useState<FileType[]>([])
  ...
  const FormSchema = Yup.object().shape({
    ...
    department_id: Yup.number().required(validationMessages.required_select('user.department')),
    location_id: Yup.number().required(validationMessages.required_select('global.province')),
    system_id: Yup.array()
  ...
  const createFormData = (values: any) => {
    ...
    formData.append('signature', selectedSignatureFiles[0] as Blob)
    return formData
  }
  ...
  useEffect(() => {
    dispatch(getALlDepartmentInToOption()).then((res) => {
  ...
  const onSelectFile = (e: React.ChangeEvent<HTMLInputElement>, type: FileType) => {
    ...
    if (newFiles) {
      ...
      if (type === 'image') {
        ...
        setSignaturePreviews(validNewFiles.map((file) => URL.createObjectURL(file)))
        setSelectedSignatureFiles(validNewFiles)
        setSignatureFileTypes(validNewFiles.map(() => 'signature'))
      }
  ...
  useEffect(() => {
    const currentFilePreviews = fileTypes.map((type, index) =>
      type === 'image' ? filePreviews[index] : signaturePreviews[index]
    )
    return () => {
      currentFilePreviews.forEach((objectUrl) => URL.revokeObjectURL(objectUrl))
    }
  }, [filePreviews, fileTypes, signaturePreviews])
    ...
    <>
      {!loader ? (
        <AddUserForm
          ...
          SignatureFileTypes={SignatureFileTypes}
