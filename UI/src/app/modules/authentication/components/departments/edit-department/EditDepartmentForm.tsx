...
const EditDepartmentForm = ({formik, directorates, renderErrorMessage, t, loading}: any) => {
  return (
    <div className='card' id='kt_profile_details_view'>
      <div className='card-header cursor-pointer'>
        <div className='card-title m-0'>
          <h3 className='fw-bolder m-0'>ثبت مدیریت جدید</h3>
        </div>
      </div>
          ...
          <div className='card-body'>
            ...
            <div className='row background-fa'>
              ...
              <div className='col-lg-4 col-md-4 col-sm-12 mb-2'>
                <label className='col-form-label label fw-bold fs-5'>
                  {t('global.directorate')}:
                  <i className='fa-solid fa-star-of-life text-danger align-text-top fs-9'></i>
                </label>
                <CustomeSelect
                  name='departmentId'
                  onChange={(directorate: any) =>
                    formik.setFieldValue('directorate_id', directorate.id)
                  }
                  options={directorates}
                  placeholder={t('global.select', {name: 'global.directorate'})}
                  value={formik.values.directorate_id}
                  className={renderErrorMessage('directorate_id') ? `validation-error-select` : ''}
                />
                {renderErrorMessage('directorate_id')}
              </div>
            </div>
