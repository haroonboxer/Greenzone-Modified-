...
import {getVehicle} from 'redux/green_zone/vehicles/VehicleSlice'
...
const DataTable: React.FC<any> = ({headers, columns, onRecordsChange}) => {
  ...
  const [vehicleSource, setVehicleSource] = useState<string>('')
  ...
  const handleSearchDriverName = useRef(
    debounce((query) => {
      setDriverName(query)
      setCurrentPage(1)
    }, 500)
  ...
  const handleSearchVehicleSource = useRef(
    debounce((query) => {
      setVehicleSource(query)
      setCurrentPage(1)
    }, 500)
  ).current
              ...
              <div className='row mb-8 col-lg-12'>
                ...
                <div className='col-lg-2 col-md-5 col-sm-4'>
                  <input
                    ...
                    onChange={(e) => handleSearchDriverName(e.target.value)}
                  />
                </div>
                <div className='col-lg-2 col-md-5 col-sm-4'>
                  <input
                ...
                <div className='col-lg-2 col-md-5 col-sm-4'>
                  <input
                    type='search'
                    placeholder={t('global.vehicleSourceBased')}
                    className='form-control form-control-sm'
                    onChange={(e) => handleSearchVehicleSource(e.target.value)}
                  />
                </div>
                <div className='col-lg-2 col-md-5 col-sm-4'>
                ...
                {!memoizedLoading &&
                  memoizedData.map((item, index) => (
                    <tr key={index} className='fs-5'>
                      ...
                      <td style={{textAlign: 'center', color: item.driverName ? '#000' : 'red'}}>
                        {item.driverName || `${t('vehicle.notSavedYet')}`}
                      </td>
