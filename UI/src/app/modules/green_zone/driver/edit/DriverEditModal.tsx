...
import { updateDriver, viewDriver } from 'redux/green_zone/driver/driverSlice'
...
const DriverEditModal = ({ show, onHide, onSuccess, driverId }: DriverEditModalProps) => {
    ...
    const FormSchema = Yup.object().shape({
        ...
        photo: Yup.mixed().nullable(), // not required in edit
    })
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const response = await dispatch(updateDriver({ id: driverId, formData }))
                if (response?.meta?.requestStatus === 'fulfilled') {
                    toast.success(t('driver.toast_update'))
                    onSuccess?.()
                    onHide()
                } else {
    ...
    useEffect(() => {
        if (show && driverId) {
            setLoading(true)
            const formData = new FormData()
            dispatch(viewDriver({ id: driverId, formData }) as any).then((res: any) => {
                if (res.meta.requestStatus === 'fulfilled') {
                    const d = res.payload.record
                    formik.setValues({
                        name: d.name || '',
                        f_name: d.f_name || '',
                        g_f_name: d.g_f_name || '',
                        phone: d.phone || '',
                        nic: d.nic || '',
                        main_province: d.main_province || '',
                        main_district: d.main_district || '',
                        main_village: d.main_village || '',
                        current_province: d.current_province || '',
                        current_district: d.current_district || '',
                        current_village: d.current_village || '',
                        photo: null,
                    })
                    setSelectedDistricts(districts.filter(dd => String(dd.provincecode) === d.main_province))
                    setSelectedCurrentDistricts(districts.filter(dd => String(dd.provincecode) === d.current_province))
                    setImagePreview(d.photo || null)
                }
                setLoading(false)
            })
        }
    }, [show, driverId, dispatch])
            ...
            <Modal.Header closeButton>
                <Modal.Title>
                    <h2 className='fw-bold m-0'>
                        <i className='fas fa-edit text-warning'></i> {t('global.edit', { name: t('driver.driver') })}
                    </h2>
                </Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <div className='d-flex justify-content-between'>
                        <button type='submit' className='btn btn-primary' disabled={loading}>
                            {loading ? t('driver.saving') : t('global.update')}
                        </button>
                        <button className='btn btn-danger' type='button' disabled={loading} onClick={() => onHide()}>
                            {t('driver.back')}
                        </button>
                    </div>
                </form>
