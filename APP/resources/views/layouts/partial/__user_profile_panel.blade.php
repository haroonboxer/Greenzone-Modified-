            ...
            <div class="symbol symbol-100 mr-5">
                <form id="avatar_store_form">
                    ...
                    <div class="image-input image-input-empty image-input-outline" id="kt_user_edit_avatar"
                        style="background-image: @if (Auth::user()->image != null) url({{ asset('storage/user_images/' . Auth::user()->image) }}) @else url({{ asset('assets/blank.png') }}) @endif">
                        <div class="image-input-wrapper"></div>
                        <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow"
                            data-action="change" data-toggle="tooltip" title=""
                            data-original-title="{{ trans('words.Change Image') }}">
                            <i class="fa fa-pen icon-sm text-muted"></i>
                            <input type="file" name="profile_avatar" accept=".png, .jpg, .jpeg" />
                            <input type="hidden" name="profile_avatar_remove" />
                        </label>
                        <span class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow"
                            data-action="cancel" data-toggle="tooltip" title="{{ trans('words.Close') }}">
                            <i class="ki ki-bold-close icon-xs text-muted"></i>
                        </span>
                        <span class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow"
                            data-action="remove" data-toggle="tooltip" title="{{ trans('words.Close') }}">
                            <i class="ki ki-bold-close icon-xs text-muted"></i>
                        </span>
                    </div>
                </form>
        ...
        <form action="{{ route('change-user-password') }}" method="POST">
            ...
            <div class="mt-4">
                <button class="btn btn-outline-dark btn-block"
                    type="submit">{{ trans('words.Change Password') }}</button>
            </div>
        </form>
        <!--begin::Separator-->
        <div class="separator separator-dashed mt-8 mb-5"></div>
