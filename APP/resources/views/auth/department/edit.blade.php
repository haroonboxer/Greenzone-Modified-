<div class="modal-header bg-color-dark">
    <h3>
        <span class="profile-main-title white-color"><i
                class="m-menu__link-icon fas fa-sitemap text-primary"></i>&nbsp;&nbsp;{{ trans('global.departmentEdit') }}</span>
    </h3>
        ...
        <div class="col-lg-12">
            <form enctype="multipart/form-data" id="requestForm" method="post">
                        ...
                        <select class="form-control select2" name="directorate" id="directorate">
                            ...
                            @foreach ($directorates as $direc)
                                <option value="{{ $direc->id }}"
                                    {{ $direc->id == $department->directorate_id ? 'selected' : '' }}>
                                    {{ $direc->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class=" col-lg-4">
                        <label for="name_da" class="title-custom">{{ trans('global.name_da') }}:
                            <span style="color:red;">*</span>
                        </label>
                        <div id="name_da" class="errorDiv">
                            <input class="form-control m-input errorDiv" type="text"
                                value="{{ $department->name_da }}" name="name_da" id="name_da"
                                placeholder="{{ trans('global.name_da') }}">
                        </div>
                        <div class="name_da error-div" style="display:none;"></div>
                    </div>
                    <div class=" col-lg-4">
                        <label for="name_en" class="title-custom">{{ trans('global.name_pa') }}:
                            <span style="color:red;">*</span>
                        </label>
                        <div id="name_en" class="errorDiv">
                            <input class="form-control m-input errorDiv" type="text"
                                value="{{ $department->name_pa }}" name="name_pa" id="name_en"
                ...
                <div class="row p-5 mt-3" style="background-color: #fafafa">
                    <div class="col-lg-12">
                        <button type="button"
                            onclick="server_request_for_storing_record('{{ route('update-department', $id) }}','requestForm','POST','response_div','submitBtn');"
